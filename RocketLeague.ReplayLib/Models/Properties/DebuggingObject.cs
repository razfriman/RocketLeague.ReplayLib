...
namespace RocketLeague.ReplayLib.Models.Properties
{
    ...
    public class DebuggingObject : IProperty
    {
        ...
        private static readonly IEnumerable<Type> PropertyTypes = typeof(DebuggingObject).Assembly.GetTypes()
        ...
        private FVector? AsVector(VectorType type)
        {
            ...
            FVector? tVector = type switch
            {
                ...
                VectorType.Quantize => _reader.SerializePropertyQuantizeVector(),
                VectorType.Single => _reader.SerializePropertyVector(),
                _ => null
            };
        ...
        */
        ...
        private string AsStaticName()
        {
            ...
            {
                uint nameIndex;
                    nameIndex = _reader.ReadPackedUInt32();
                if (_reader.IsError || !_reader.AtEnd())
                {
                    return null;
                }
        ...
        private object[] AsArray()
        {
            ...
            var totalElements = _reader.ReadPackedUInt32();

            var data = new object[totalElements];

            while (true)
            {
                var index = _reader.ReadPackedUInt32();

                if (index == 0)
                {
                    if (_reader.GetBitsLeft() == 8)
                    {
                        var terminator = _reader.ReadPackedUInt32();
                ...
                while (true)
                {
                    ...
                    var handle = _reader.ReadPackedUInt32();
                    ...
                    var numBits = _reader.ReadPackedUInt32();

                    debuggingHandle.NumBits = numBits;

                    var obj = new DebuggingObject();

                    using var tempReader = _reader.GetNetBitReader((int) numBits);
        ...
        private uint? AsPropertyObject()
        {
            ...
            var obj = _reader.SerializePropertyObject();
        ...
        private FRepMovement AsFRepMovement(VectorQuantization vectorQuantization)
        {
            ...
            var movement = _reader.SerializeRepMovement(vectorQuantization);
