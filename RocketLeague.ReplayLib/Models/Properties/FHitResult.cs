...
namespace RocketLeague.ReplayLib.Models.Properties
{
    ...
    public class FHitResult : IProperty
    {
        ...
        public void Serialize(NetBitReader reader)
        {
            ...
            var bImpactPointEqualsLocation = flags[2];
            var bImpactNormalEqualsNormal = flags[3];

            // Often times the indexes are invalid, use that as an optimization
            var bInvalidItem = flags[4];
            var bInvalidFaceIndex = flags[5];
            var bNoPenetrationDepth = flags[6];
            var bInvalidElementIndex = flags[7];
            ...
            Distance = (float) (ImpactPoint - TraceStart)?.Size();
